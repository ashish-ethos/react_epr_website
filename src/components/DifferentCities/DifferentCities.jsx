...
function DifferentCities() {
  ...
  useEffect(() => {
    setTimeout(() => {
      setIsLoading(false);
      setIsVisible(true);  // Trigger existing animations after loading
    }, 1500);
  }, []);

  const LoadingSkeleton = () => (
    <div className="relative h-auto overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-black py-4 sm:py-8">
      {/* Background placeholders (keep existing absolute divs for consistency, but pulse them */}
      <div className="absolute inset-0 opacity-20 animate-pulse">
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-yellow-600/10 to-amber-600/10"></div>
        <div className="absolute bottom-0 right-0 w-80 sm:w-[600px] h-80 sm:h-[600px] bg-gradient-to-l from-amber-500/10 to-yellow-500/10 rounded-full blur-3xl"></div>
      </div>
      <div className="absolute inset-0 overflow-hidden pointer-events-none animate-pulse">
        {[...Array(15)].map((_, i) => (
          <div
            key={i}
            className="absolute w-1 sm:w-2 h-1 sm:h-2 bg-yellow-400/20 rounded-full"
            style={{
      ...
      <div className="relative z-10 h-auto py-4 sm:py-0 flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8">
        <div className="max-w-full w-full mx-auto text-center">
          ...
          <div className="inline-flex items-center gap-2 mb-6 sm:mb-8">
            <div className="w-8 sm:w-12 h-0.5 bg-[#444] rounded-full animate-pulse"></div>
            <div className="w-4 h-4 bg-[#444] rounded animate-pulse"></div> {/* Icon placeholder */}
            <div className="w-8 sm:w-12 h-0.5 bg-[#444] rounded-full animate-pulse"></div>
          </div>
          <div className="mb-6 sm:mb-8">
            <div className="h-8 w-48 bg-[#444] rounded animate-pulse mb-2 mx-auto"></div> {/* First line */}
            <div className="h-8 w-32 bg-[#444] rounded animate-pulse mx-auto"></div> {/* Second line */}
          </div>

          {/* Description skeleton */}
          <div className="max-w-full sm:max-w-4xl mx-auto mb-8 sm:mb-12">
            <div className="bg-black/20 backdrop-blur-md rounded-2xl p-4 sm:p-8 shadow-2xl border border-yellow-400/20 animate-pulse">
              <div className="h-4 w-full bg-[#444] rounded mb-2"></div>
              <div className="h-4 w-3/4 bg-[#444] rounded mb-2"></div>
          ...
          <div className="mb-8 sm:mb-12">
            <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 mb-6 sm:mb-8">
              {Array.from({ length: 4 }).map((_, index) => (
                <div
                  key={index}
                  className="bg-black/20 backdrop-blur-sm rounded-xl p-2 border border-yellow-400/20 animate-pulse w-full sm:w-[230px]"
                  style={{ animationDelay: `${index * 200}ms` }}
                >
                  {/* Image placeholder */}
                  <div className="w-full h-28 sm:h-32 bg-[#444] rounded-lg mb-3"></div>
                  {/* Content placeholders */}
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="h-4 w-20 bg-[#444] rounded mb-1"></div> {/* Name */}
                      <div className="flex items-center gap-1">
                        <div className="w-4 h-4 bg-[#444] rounded-full"></div> {/* Icon */}
                        <div className="h-3 w-12 bg-[#444] rounded"></div> {/* Growth */}
                      </div>
                    </div>
                    <div className="w-5 h-5 bg-[#444] rounded animate-pulse"></div> {/* Right icon */}
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Button skeleton */}
          <div className="h-12 w-64 bg-[#444] rounded-2xl border border-[#c08830]/50 animate-pulse mx-auto"></div>
        </div>
      </div>
    </div>
  );
