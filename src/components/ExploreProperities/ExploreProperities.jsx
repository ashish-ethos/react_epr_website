...
const ExploreProperties = ({ filters = {} }) => {
  ...
  const [searchTerms, setSearchTerms] = useState(() => {
    ...
    return search
      ? search.split(" ").filter((term) => term.trim().length > 0)
      : [];
  });
  ...
  const tabs = [
    ...
    },
    {
      key: "villa",
      label: "Villa",
      count: properties.filter((p) => p.type.toLowerCase().includes("villa"))
        .length,
    },
    {
      key: "office",
      label: "Office",
      count: properties.filter((p) => p.type.toLowerCase().includes("office"))
        .length,
    },
    {
      key: "studio",
      label: "Studio",
      count: properties.filter((p) => p.type.toLowerCase().includes("studio"))
        .length,
    },
    // { key: 'plot', label: 'Plot', count: properties.filter(p => p.type.toLowerCase().includes('plot')).length },
    {
      key: "shop",
      label: "Shop",
      count: properties.filter((p) => p.type.toLowerCase().includes("shop"))
        .length,
    },
  ];
  ...
  const normalizedFilters = useMemo(() => {
    ...
    return {
      search:
        debouncedSearch || f.search
          ? String(debouncedSearch || f.search)
              .trim()
              .toLowerCase()
          : "",
  ...
  const handleTabChange = (tabKey) => {
    ...
    const url = queryString
      ? `/explore-properties?${queryString}`
      : "/explore-properties";
    navigate(url);
  };
  ...
  const getCombinedFiltered = () => {
    ...
    if (type) {
      ...
      list = list.filter(
        (p) =>
          p._type.includes(t) || p._name.includes(t) || p._location.includes(t)
      );
    }
    if (city) {
  ...
  const filteredProperties = useMemo(getCombinedFiltered, [
    activeTab,
    normalizedFilters,
  ]);
    ...
    const handlePrint = async () => {
      ...
      try {
        ...
        tempContainer.innerHTML = `
          ...
          <h2 style="font-size: 24px; font-weight: bold; color: #c2c6cb; margin-bottom: 10px;">${
            property.name
          }</h2>
          <p style="font-size: 18px; color: #c99913; font-weight: bold; margin-bottom: 10px;">${
            property.price
          }</p>
        ...
        alert(
          "Failed to generate PDF. Please ensure images are loaded and try again."
        );
      } finally {
        setIsPrinting(false);
      }
            ...
            <div className="relative h-48 sm:h-64 md:h-72 overflow-hidden">
              ...
              <div className="absolute top-4 left-4 flex flex-wrap gap-2">
                {property.options.map((option, idx) => (
                  <span
                    key={option + idx}
                    className={`px-3 py-1 rounded-full text-xs font-bold backdrop-blur-md ${getOptionColor(
                      option
                    )} shadow-lg`}
                  >
                    {option}
              ...
              <div className="absolute bottom-4 flex items-center w-full justify-between px-4 space-x-3 mobile-share-section">
                <div className="bottom-left-image-stats flex items-center space-x-3">
                  <div className="flex items-center space-x-1 bg-[#333]/80 backdrop-blur-md rounded-full px-3 py-1">
                    ...
                    <span className="text-sm font-medium text-[#c2c6cb]">
                ...
                <div className="bottom-right-image-stats flex items-center space-x-3">
                  <div
                    ...
                    className="cursor-pointer flex items-center space-x-1 bg-[#333]/80 backdrop-blur-md rounded-full px-3 py-1 hover:bg-[#444]/80 transition-all"
                  >
                  ...
                  <div
                    className={`
                                        ...
                                        ${
                                          (shareCounts[property.id] || 0) > 0
                                            ? "bg-gradient-to-r from-amber-700/50 to-amber-600/50 text-amber-200"
                                            : "bg-[#333] text-[#c2c6cb]"
                                        }
                  ...
                  >
                    <span>
                      {shareCounts[property.id] || 0}{" "}
                      {(shareCounts[property.id] || 0) === 1
                        ? "Shares"
                        : "Share"}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-2 mt-4">
                    ...
                    >
                      <div className="flex items-center space-x-2 text-[#c2c6cb] hover:text-blue-400 cursor-pointer">
                        ...
                        <span className="text-xs font-[Montserrat] ml-4">
                          Facebook
                        </span>
                      </div>
                    </FacebookShareButton>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div className="space-y-4">
                <div className="bg-[#333] rounded-xl p-4 border border-[#ffffff38]">
                  ...
                  <p className="text-[#c2c6cb] text-sm leading-relaxed fontFamily-bebas">
                    {property.location}
                  </p>
                </div>
                <div className="bg-[#333] rounded-xl p-4 border border-[#ffffff38]">
                  ...
                  <p className="text-[#c2c6cb] text-lg font-medium fontFamily-Content">
                    {property.size}
                  </p>
                </div>
              </div>
              <div className="space-y-4">
                <div className="bg-[#333] rounded-xl p-4 border border-[#ffffff38]">
                  <h3 className="text-base font-semibold text-[#c2c6cb] fontFamily-Content mb-2">
                    Description
                  </h3>
                  <p className="text-[#c2c6cb] text-sm leading-relaxed fontFamily-bebas">
                ...
                <div className="get-in-touch-section border border-[#ffffff38] p-2 sm:p-3 rounded-xl shadow-md">
                  <h3 className="text-base font-semibold text-[#c2c6cb] mb-3 fontFamily-bebas">
                    Get in Touch
                  </h3>
                  <div className="flex flex-col sm:flex-row gap-3 tablet-contact-buttons">
                    <CustomButton
                      onClick={() =>
                        (window.location.href = "tel:+918744964496")
                      }
                    ...
                    <CustomButton
                      onClick={() => {
                        const message = encodeURIComponent(
                          `Hello, I am interested in the property: ${property.name}. Please share more details.`
                        );
                        window.open(
                          `https://wa.me/918744964496?text=${message}`,
                          "_blank",
                          "noopener,noreferrer"
                        );
                      }}
                    ...
                    <CustomButton
                      onClick={() =>
                        (window.location.href =
                          "mailto:info@ethosprorealtors.com")
                      }
                      className="flex-1 bg-[#333] hover:bg-[#444] text-[#c2c6cb] font-semibold py-3 px-4 rounded-xl transition-all duration-200 hover:scale-105 hover:shadow-lg flex items-center justify-center group border border-[#ffffff38]"
                    >
          ...
          <div className="p-4">
            ...
            <div className="space-y-1 sm:space-y-2 mb-3 sm:mb-4">
              <div className="flex items-start space-x-2 text-[#c2c6cb]">
                ...
                <p className="text-xs sm:text-sm leading-relaxed line-clamp-2 font-[sans-serif]">
                  {property.location}
                </p>
              </div>
            ...
            <div className="mb-3 sm:mb-4">
              <p className="text-base sm:text-xl font-bold text-amber-400 line-clamp-1">
                {property.price}
              </p>
            </div>
            <div className="flex space-x-1 sm:space-x-2 flex-wrap sm:gap-2">
              {[
                ...
                },
                {
                  ...
                  label: "WhatsApp",
                  href: `https://wa.me/918744964496?text=${encodeURIComponent(
                    `Hello, I am interested in the property: ${property.name}. Please share more details.`
                  )}`,
                  target: "_blank",
                },
  ...
  const NoResults = () => (
    <div className="text-center py-8 sm:py-12 px-4 bg-[#333]/80 backdrop-blur-sm rounded-2xl shadow-lg max-w-full sm:max-w-2xl mx-auto animate-fadeIn border border-[#ffffff38]">
      ...
      <p className="text-[#c2c6cb] text-sm sm:text-lg font-[Inter] max-w-full sm:max-w-md mx-auto">
        There are no properties available for the selected category. Try another
    ...
    <div className="min-h-screen bg-[#2d2d2d] py-8 sm:py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-full sm:max-w-7xl mx-auto mb-8 sm:mb-12 text-center">
        ...
        <p className="mobile-subtitle-text text-[#c2c6cb] text-sm font-[sans-serif] sm:text-lg max-w-full sm:max-w-2xl mx-auto mt-2 font-[Inter] explore-properties-maincontent">
          Discover a diverse collection of premium properties, from luxurious
          residences to high-end commercial spaces. Browse through the latest
          listings, featuring stunning architecture, prime locations, and
          exceptional investment opportunities. Discover the perfect investment
          or dream residence with ease.
        </p>
      </div>
        ...
        <div className="flex bg-[#333] mobile-tab-container overflow-x-auto sm:flex-wrap justify-start sm:justify-center gap-2 sm:gap-3 p-2 bg-[#333]/80 backdrop-blur-sm rounded-2xl shadow-lg scrollbar-hidden border border-[#ffffff38]">
          {tabs.map((tab) => (
            <CustomButton
              ...
              className={`px-4 sm:px-6 py-2 sm:py-3 font-semibold transition-all cursor-pointer duration-300 transform hover:scale-105 text-sm sm:text-base whitespace-nowrap border border-[#ffffff38] ${
                activeTab === tab.key
            ...
            >
              {tab.label}
              <span
                className={`ml-1 sm:ml-2 px-1 sm:px-2 py-0.5 sm:py-1 rounded-full text-[10px] border  sm:text-xs ${
                  activeTab === tab.key
