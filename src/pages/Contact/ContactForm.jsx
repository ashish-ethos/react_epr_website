...
const ContactForm = ({ onSubmitSuccess }) => {
  ...
  const handlePhoneChange = (value, countryData, e, formattedValue) => {
    const fullPhone = formattedValue
      ? formattedValue.replace(/[\s-]/g, "")
      : `+${countryData.dialCode}${value}`;
    if (fullPhone.startsWith("+91") && fullPhone.length !== 13) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        err.message ||
          "Failed to submit the form. Check your network or server configuration."
      );
    } finally {
      setLoading(false);
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <TextArea
                ...
                className="w-full px-4 py-3 border border-[#ffffff38]
                    focus:border-[#c99913] focus:ring-4 focus:ring-[#c99913]/20
                    transition resize-none placeholder-[#c2c6cb]
                    bg-[#333] text-[#c2c6cb] overflow-hidden"
              />
            </div>
            ...
            <div className="flex justify-center">
              <button
                type="submit"
                disabled={loading}
                className={`w-auto px-6 py-2 cursor-pointer rounded-lg text-[#c2c6cb] font-semibold transition border border-[#ffffff38] ${
                  loading
                    ? "bg-[#444] cursor-not-allowed"
                    : "bg-[#333] hover:bg-[#444] focus:ring-4 focus:ring-[#c99913]/20"
                }`}
              >
                {loading ? (
